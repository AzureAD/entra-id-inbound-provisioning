...
| 3. Use a certificate or client secret for authentication. | Configure authentication in **Step 1 — Connect**. Supports client secret and client certificate (.pem/.pfx). | For production, use certificate-based authentication. See [Configure client certificate for service principal authentication](https://learn.microsoft.com/en-us/entra/identity/app-provisioning/inbound-provisioning-api-powershell#configure-client-certificate-for-service-principal-authentication). |
